...
[~] admin ui, product editor: source of delivery "leveranskälla" -- instead of "Tillgångs-ID" just ID is enough; do not need a sub panel either, just put them to the right of the other controls, ID and copy icon button URL and visitor. sans serif, normal text color. The header of the File panel is ugly, make it sans serif uppercase, as a title band inside the subpanel with bottom border right below the title "FILE". I want this pattern of titled subpanel throughout the admin ui where applicable. STATUS 2026-03-31: FILE title band + ID label done; remaining layout simplification (drop subpanel / right-side inline arrangement) pending.
...
[x] BUG (P0, codex regression): /api/digital/download and /digital/[slug] routes had `runtime = "nodejs"` and `import path from "node:path"` — will 500 on CF Workers. Fixed: converted to edge runtime. Prio: critical. Impact: all digital downloads broken.
