...
[ ] admin ui products, edit product: main title is gray, make it amber and sans serif. "Nedladdningsprodukt — redigera detaljerna nedan i den här panelen." remove this, superflous. "Kategorier: Digital file, Download, Video": make these pills and sans serif.
[ ] shop, single product, digital download: honor line breaks and spacing by making the paragraph a pre-tag but with the site normal serif instead.
[ ] shop, single product, digital download, "Hämta gratis", "Det gick inte att starta betalningen." which is a misleading message and there should be some access granted instead, the non-purchase is not in /inventory. The same for two digital download products.
[ ] shop, products cards should have the price aligned right below the colored type pill "Digital fil", the "Visa & Köp" button can go away as the whole card will link there instead. "Digital fil" should be a pill, color coded and sans serif according to product types normal coloring from admin ui. Link the whole card to the same destination as "Visa & Köp", amber card border 2px solid on hover. Set the header type for the card as h2 and make it a couple of points smaller.
...
[ ] admin ui, products list: need configuration filter pill - unclear why some products need configuration, do they actually? If so, state which / where / why when opening the product editor.
[ ] admin ui, product editor: replace the "Ta bort", "remove" link for a product editors top right corner with a trash can icon button (not red, normal colors).
