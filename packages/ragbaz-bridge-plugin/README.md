# RAGBAZ Bridge (WordPress GraphQL helper)
...
1. Copy `ragbaz-bridge.php` to `wp-content/plugins/` (or `wp-content/mu-plugins/` to keep it always-on).
...
```bash
npm install
npm run build --workspace ragbaz-bridge-plugin
```

Output: `dist/ragbaz-bridge.zip`
