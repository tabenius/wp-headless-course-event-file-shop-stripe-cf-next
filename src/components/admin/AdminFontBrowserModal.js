      ...
      >
        <div className="flex-1 min-w-0">
          <div className="mb-1 flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-300">
            {font.family}
            {isVar && (
              <span className="rounded bg-slate-600 px-1.5 py-0.5 text-[10px] text-slate-100">
                Variable
              </span>
            )}
            {usedByRoles &&
              usedByRoles.map((r) => (
                <span
                  key={r}
                  className="rounded bg-slate-600 px-1.5 py-0.5 text-[10px] text-slate-100"
                >
                  {r}
                </span>
              ))}
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          {dlError && (
            <span className="text-[10px] text-red-300">Error: {dlError}</span>
          )}
          <div className="flex items-center gap-2">
            {isDl ? (
              <span className="rounded bg-green-900/40 px-2 py-1 text-xs font-medium text-green-200">
                Downloaded
              </span>
            ) : (
              <button
                onClick={() =>
                  isVar
                    ? onDownload(font.family)
                    : onOpenWeightPicker(font.family)
                }
                disabled={isDling}
      ...
      <div className="flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-slate-600 bg-slate-800 text-slate-100 shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-600 px-6 py-4">
          <h2 className="text-lg font-bold text-slate-100">
            Font Browser{" "}
            <span className="font-normal text-slate-300">({role})</span>
          </h2>
          <button
            onClick={onClose}
            ...
            <>
              {visible.map((font) => (
                <FontRow
                  ...
                  usedByRoles={usedFonts
                    .filter((u) => u.family === font.family)
                    .map((u) => u.role)}
          ...
          <div className="absolute inset-x-0 bottom-0 z-10 animate-in border-t border-slate-600 bg-slate-800 p-6 shadow-up-lg slide-in-from-bottom">
            <div className="flex justify-between items-center mb-4">
              <p className="font-bold text-slate-100">
                Weights for {weightPickerFamily}
              </p>
              <button
