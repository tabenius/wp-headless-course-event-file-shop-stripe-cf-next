export const ADMIN_ENV_GROUPS = [
  {
    ...
    vars: [
      ...
      { label: "Admin password hash", names: ["ADMIN_PASSWORD_HASH"], secret: true, hint: "bcrypt hash of admin password" },
      { label: "Auth secret", names: ["AUTH_SECRET"], secret: true },
      { label: "JWT secret", names: ["JWT_SECRET"], secret: true, hint: "Signs session tokens" },
  ...
  },
  {
    ...
    label: "WordPress",
    vars: [
      { label: "Site URL", names: ["NEXT_PUBLIC_WORDPRESS_URL", "WORDPRESS_API_URL"] },
      { label: "GraphQL endpoint", names: ["WORDPRESS_GRAPHQL_ENDPOINT"] },
      { label: "WP app password", names: ["WORDPRESS_APPLICATION_PASSWORD", "WORDPRESS_GRAPHQL_APPLICATION_PASSWORD"], secret: true },
      { label: "WP app password user", names: ["WORDPRESS_APPLICATION_PASSWORD_USER"] },
      { label: "Faust secret", names: ["FAUST_SECRET_KEY", "FAUSTWP_SECRET_KEY"], secret: true },
    ],
  },
  {
    id: "r2s3",
    ...
    vars: [
      ...
      { label: "Account ID", names: ["CLOUDFLARE_ACCOUNT_ID", "CF_ACCOUNT_ID"] },
      { label: "S3 endpoint", names: ["S3_ENDPOINT"] },
      { label: "Bucket name", names: ["S3_BUCKET_NAME", "CF_R2_BUCKET_NAME"] },
      { label: "Region", names: ["S3_REGION"] },
      { label: "Public base URL", names: ["S3_PUBLIC_URL", "CF_R2_PUBLIC_URL"] },
      { label: "Access Key ID", names: ["S3_ACCESS_KEY_ID", "CF_R2_ACCESS_KEY_ID"] },
      { label: "Secret Access Key", names: ["S3_SECRET_ACCESS_KEY", "CF_R2_SECRET_ACCESS_KEY"], secret: true },
  ...
  },
  {
    ...
    vars: [
      ...
      { label: "Publishable key", names: ["NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY"] },
      { label: "Default currency", names: ["DEFAULT_COURSE_FEE_CURRENCY"] },
      { label: "Webhook secret", names: ["STRIPE_WEBHOOK_SECRET"], secret: true },
    ],
  },
  {
    id: "cloudflare",
    ...
    vars: [
      ...
      { label: "API token", names: ["CF_API_TOKEN", "CLOUDFLARE_API_TOKEN"], secret: true },
