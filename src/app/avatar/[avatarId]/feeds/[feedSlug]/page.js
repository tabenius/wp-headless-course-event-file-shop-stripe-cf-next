...
export default async function AvatarCustomFeedPage({ params: paramsPromise }) {
  const params = await paramsPromise;
  const avatarIdRaw =
    typeof params?.avatarId === "string" ? params.avatarId : "";
  const feedSlugRaw =
    typeof params?.feedSlug === "string" ? params.feedSlug : "";
  const requestedHandle = decodeSegment(avatarIdRaw);
  const feedSlug = decodeSegment(feedSlugRaw).toLowerCase();

  if (!requestedHandle || !feedSlug) notFound();

  if (feedSlug === getDefaultFeedSlug()) {
    redirect(`/avatar/${encodeURIComponent(requestedHandle)}/default`);
  }
  if (feedSlug === getCompositeFeedSlug()) {
    redirect(`/avatar/${encodeURIComponent(requestedHandle)}/composite`);
  }

  const session = await auth();
  if (!session?.user?.email) {
    redirect(
      `/auth/signin?callbackUrl=${encodeURIComponent(`/avatar/${requestedHandle}/feeds/${feedSlug}`)}`,
    );
  }
  ...
  if (!ownAvatar?.id) {
    return (
      <section className="max-w-3xl mx-auto px-6 py-16 space-y-4">
        <h1 className="text-3xl font-bold">Avatar Feed</h1>
