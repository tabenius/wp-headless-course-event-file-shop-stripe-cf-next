...
export const metadata = {
  title: "Avatar Composite Feed",
};

export default async function AvatarCompositeFeedPage({ params: paramsPromise }) {
  ...
  if (!session?.user?.email) {
    redirect(
      `/auth/signin?callbackUrl=${encodeURIComponent(`/avatar/${requestedHandle}/composite`)}`,
    );
  }
  ...
  if (!ownAvatar?.id) {
    return (
      <section className="max-w-3xl mx-auto px-6 py-16 space-y-4">
        <h1 className="text-3xl font-bold">Composite Feed</h1>
  ...
  const feedSlug = getCompositeFeedSlug();
