...
- 🔒 NOTE (for Codex): The following files are reserved for Claude — do NOT touch until the lock is removed in AGENTS.md: `ImageGenerationPanel.js`, `generate-image/route.js`, `api/chat/route.js`, `src/lib/ai.js`, `src/lib/imageQuota.js`, `tests/generate-image.test.js`.
- Claude plans: image generator polish (aspect-ratio thumbnails, copy-prompt button, count=1 option, elapsed-time counter) + chat fixes (payments `rows` crash, deduplicate imageSystemPrompt, cap history to 10 turns). Full specs in AGENTS.md under "[Claude] Upcoming".
