...
- Docs lock protocol established: `docs.lock.pid` advisory lock via `scripts/docs-lock.mjs` (acquire/release/check). Both agents must acquire before editing AGENTS.md or coop file. Full protocol in AGENTS.md "Shared-doc lock protocol" section.
- Code review (Claude → Codex): StatsChart and ProductSection refactors reviewed — 3 follow-up fixes logged in AGENTS.md: `formatHour` UTC bug, workers-hint i18n, `ProductSection` renderItem pattern. See AGENTS.md "follow-up fixes needed" section.
