...
- **2026-03-30 (Codex)**: Moved the Info-page torus canvas mount from `AdminSandboxTab` to the top of `AdminInfoHubTab`, so it now appears above the Info section tabs across the whole `/admin/info` view (not only Overview).
- **2026-03-30 (Codex)**: Updated admin env-status behavior so secret env vars now return values to authenticated admin requests; Info → Storage env panel still masks by default but now reveals actual values on “Show” and supports copy for revealed secrets (including R2 keys).
- **2026-03-30 (Codex)**: Unified Resend sender env handling via `resendConfig` helper so `RESEND_FROM_EMAIL`, `RESEND_FROM`, and `EMAIL_FROM` are accepted consistently across send-paths and admin checks; updated env catalog + user-facing copy to reduce false "not configured" states.
- **2026-03-30 (Codex)**: Fixed KV configuration detection to accept Cloudflare env aliases (`CLOUDFLARE_ACCOUNT_ID/CF_ACCOUNT_ID`, `CF_API_TOKEN/CLOUDFLARE_API_TOKEN`) and corrected misleading admin copy: web-vitals logging is independent from GraphQL availability logging and warnings now indicate full KV config requirements.
- **2026-03-30 (Codex)**: Added explicit Cloudflare KV runtime diagnostics to GraphQL/Page Performance admin APIs and UI so warnings now include exact missing runtime key aliases and build-bypass state instead of generic "not fully configured" messaging.
